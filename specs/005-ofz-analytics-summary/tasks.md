...
- [X] T039 Проверить вручную вкладку `Активность ОФЗ`: summary, filters, heatmap, weak liquidity, detail charts, structured JSON
...
- [X] T042 Разделить вкладку `Активность ОФЗ` на режимы `Обзор`, `Сигналы`, `Детализация` с динамическим ContentControl
